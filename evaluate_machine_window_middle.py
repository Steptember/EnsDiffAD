...
for iteration in os.listdir("train_result"):
    ...
    for subset_name in os.listdir(f"train_result/{iteration}/"):
        if args.dataset == "GCP" and "service" not in subset_name:
            continue
        elif args.dataset == "MSL" and "MSL" not in subset_name:
            continue
        elif args.dataset == "SMAP" and "SMAP" not in subset_name:
            continue
        elif args.dataset == "PSM" and "PSM" not in subset_name:
            continue
        elif args.dataset == "SWaT" and "SWaT" not in subset_name:
            continue
        data_id = subset_name.split("_unconditional")[0]

        if "unconditional_True" in subset_name:
            unconditional = True
        else:
            unconditional = False

        split = 4
        diffusion_step = int(subset_name.split("diffusion_step_")[-1])
        ...
        tmp = torch.load(f"{base_folder}/best-model.pth", map_location=args.device)
