...
class TrainData(Dataset):
    ...
    def __init__(self, file_path, test_path, window_length=100,split=4,mask_ratio=0.5):
        ...
        if "SWaT" in file_path:
            scaler = MinMaxScaler()
            self.data = scaler.fit_transform(self.data[:length, :])
        else:
    ...
    def get_mask(self, observed_mask, strategy_type):
        ...
        num_features = observed_mask.shape[1]
        total_points = length * num_features
        num_masked = int(total_points * self.mask_ratio)

        if strategy_type == 0:
            indices = torch.randperm(total_points)[:num_masked]
            mask = mask.reshape(-1)
            mask[indices] = 1
            mask = mask.reshape(length, num_features)
...
def get_dataloader(train_path, test_path, label_path,batch_size = 32,window_split=1,split=4,mask_ratio=0.5):
    ...
    for i in tqdm(range(0,100)):
        mask_list.append(get_mask(observed_mask,mask_ratio=0.3))
...
if __name__ == "__main__":
    train_loader, valid_loader, test_loader1, test_loader2 = get_dataloader(
        "data/Machine/MSL_train.pkl",
        "data/Machine/MSL_test.pkl",
        "data/Machine/MSL_test_label.pkl",
        split=8
    )
