...
class diff_CSDI(nn.Module):
    def __init__(self, config, inputdim=2):
        ...
        self.strategy_embedding = nn.Embedding(
            2, config_diff['diffusion_embedding_dim']
        )
...
class ResidualBlock(nn.Module):
    def __init__(self, config_itran,side_dim, channels, diffusion_embedding_dim, nheads):
        super().__init__()
        ...
        self.get_itran=iTransformer(config_itran)
    ...
    def forward_feature(self, y, base_shape):
        ...
        y = self.get_itran(y.permute(0,2,1)).permute(0,2,1)
        y = y.reshape(B, channel, K, L).reshape(B, channel, K * L)
