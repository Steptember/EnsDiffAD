...
def train(
    ...
    for epoch_no in range(0,500):
        ...
        with tqdm(train_loader) as it:
            for batch_no, train_batch in enumerate(it, start=1):
                ...
                for i,(model,optimizer) in enumerate(zip(models,optimizers)):
                    ...
                    loss, weights = model.forward(train_batch, is_train=1, weights = weights)
                    losses.append(loss)
        ...
        mse_score = validation(models[-1], valid_loader)
        if mse_score < best_mse_score:
            stop_counter = 0
            best_mse_score = mse_score
            print("Best MSE score updated:", best_mse_score)
            torch.save(models[-1].state_dict(), foldername + "/best-model.pth")
        else:
            stop_counter += 1
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader1, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                samples[:, :, samples_length // 4: samples_length // 2, :] = samples2[:, :,
                                                                             samples_length // 4: samples_length // 2,
                                                                             :]
                samples[:, :, samples_length - samples_length // 4:, :] = samples2[:, :,
                                                                          samples_length - samples_length // 4:, :]
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader1, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                #samples = samples.squeeze() * eval_points
                samples_length = samples.shape[1]
                ...
                mae_total += mae_current.sum().item()
                evalpoints_total += torch.ones_like(samples).sum().item()
                ss_total=torch.sum((c_target-c_target.mean())**2)
                ss_residual=torch.sum((c_target-samples_median)**2)
