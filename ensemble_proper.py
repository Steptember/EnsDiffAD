...
from slidingWindows import find_length
import numpy as np
...
def merge(pkl_path,data_id,machine_number = ""):
    ...
    head_middle = result[-4]
    all_gen_middle = result[-5]
...
def ensemble(pkl_path, data_id, ensemble_strategy_list = [],last_step_threshold = 0.005,compute_abs=True,compute_sum=True,machine_number=""):
    if data_id == "GCP":
    ...
    score = origin_prediction.numpy()
    label_np = label[:len(score)].numpy().astype(int)


    window_size = find_length(score)
    grader = metricor()
    R_AUC_PR = grader.RangeAUC(labels=label_np, score=score, window=window_size)
    ...
    return [best_p,best_r,best_f,best_add, R_AUC_PR, best_ensemble_threshold] + [torch.std(torch.Tensor(average_residual_list)).item()] + average_residual_list,\
...
def compute_one_strategy(data_id,strategy_name,ensemble_strategy_list,csv_writer,last_step_threshold=0.02):
    ...
    if data_id == "GCP":
        ...
        threshold_dict = {}
        csv_reader = csv.reader(open("score/GCP/infor.csv"))
