...
class CSDI_base(nn.Module):
    ...
    def calc_loss(
            self, observed_data, cond_mask, observed_mask, side_info, is_train, strategy_type, set_t=-1, weights=None
    ):
        B, K, L = observed_data.shape
        if is_train != 1:  # for validation
            t = (torch.ones(B) * set_t).long().to(self.device)
        ...
        target_mask = observed_mask - cond_mask
        residual = (noise - predicted) * target_mask
        pt = (residual ** 2).sum(dim=[1, 2], keepdim=True) / (target_mask.sum(dim=[1, 2], keepdim=True) + 1e-5)

        p = torch.exp(-pt)  # [B, K, L]
        if weights is None:
            weights = p.detach()
            p = (1 - p) ** 2.0
            loss = p.clone().detach() * pt
        else:
            weights = torch.cat([weights, p], 2)
            p = torch.mean(weights, 2).detach()  # [B,K,L]
            p = (1 - p) ** 2.0
            loss = p.clone().detach() * pt
    ...
    def forward(self, batch, is_train=1, weights = None):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        return loss_func(observed_data, cond_mask, observed_mask, side_info, is_train, weights = weights, strategy_type = strategy_type)
    ...
    def ddim_evaluate(self, batch, n_samples, ddim_eta=1, ddim_steps=10):
        ...
        with torch.no_grad():
            ...
            samples = self.ddim_impute(observed_data, cond_mask, side_info, n_samples, ddim_eta=ddim_eta,
